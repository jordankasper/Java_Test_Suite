...
class ResinTest(TestCase):
    ...
    @java
    @java_test_webapp
    @resin(jvm_args=['-Dnewrelic.config.auto_instrument=false'])
    def test_manual_rum(self):
        ...
        self.assertTrue(
            self.test_env.resin.get_path_and_shutdown('/java_test_webapp/RumServlet', data=data, headers=headers, response_size=response_size))
        actual_html = self.test_env.resin.get_html().replace(b'\r', b'').decode("utf-8")
    ...
    def test_mobile_apm(self):
        ...
        self.assertTrue(
            self.test_env.resin.get_path_and_shutdown('/java_test_webapp/TestServlet2', data=data, headers=headers, response_size=response_size))

        actual_html = self.test_env.resin.get_html().replace(b'\r', b'')

        self.assertEqual(expected_html, actual_html, "HTML response does not match")

        self.assertTrue(
            self.test_env.resin.get_response_header('TEST_HEADER', regex="TEST_HEADER CONTENT", deobfuscate=False))

        regex = '\[\"{}\",\"WebTransaction\\\/Servlet\\\/TestServlet2\\\",[0-9][\d]*\.[\d]+,\d+.\d+,10\]'.format(CollectorHTTPServer.AGENT_CROSS_PROCESS_ID)
        self.assertTrue(
            self.test_env.resin.get_response_header('X-NewRelic-App-Data', regex=regex, deobfuscate=True))
    ...
    def test_custom_extension(self):
        self.assertTrue(
                        self.test_env.resin.get_path_and_shutdown('/java_test_webapp/CustomTestServlet'))
    ...
    @java
    @java_test_webapp
    @resin(select={'version' : [4.0]})
    def test_async(self):
        self.assertTrue(
                        self.test_env.resin.get_path_and_shutdown('/java_test_webapp/AsyncServlet'))
...
#    @resin
    def test_metric(self):
        self.assertTrue(self.test_env.resin.get_path_and_shutdown('/java_test_webapp/TestServlet'))
    ...
    @java
    @java_test_webapp
    @resin(jvm_args=['-Dnewrelic.config.transaction_tracer.transaction_threshold=1.0'])
    def test_transaction_trace(self):
        self.assertTrue(
            self.test_env.resin.get_path_and_shutdown(
